...
[2026-09-04 16:18 UTC] HUMAN RULING — D-01 DECIDED (option b: precision reading of G7; OpenFlights lookup demoted to cross-check; icao match tier to be deleted; curated designator-keyed nation table to be built attended). D-02 DECIDED (option C: OpenSky ADS-B as primary source). FIX-02 remains BLOCKED-NEEDS-HUMAN until the curated table exists and FIX-02 is re-run (Round 2, FIX-06); Round-1 nation-level reporting embargo stays in force. Recorded in DECISIONS.md.
[2026-09-04 16:18 UTC] ROUND 02 DRAFTED (human with Claude) — rounds/round-02-opensky-extract/ROUND_02.md. Header status NEEDS-HUMAN: tasks ATT-00/01/02 are attended-only (T8; web access for ATT-02) and must be run with the human present before /run-analysis is invoked for FIX-03 onward. Active round becomes rounds/round-02-opensky-extract/ROUND_02.md once ATT-01 has cached extracts in data/raw/opensky/.
