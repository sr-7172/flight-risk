...
## D-01 — RULING (human, 2026-09-04) — DECIDED

**Ruling.** Option (b): replace the lookup. Specifically:
1. G7 is read as the **precision** reading from now on. The coverage
   reading is retired and may not be used to pass the gate.
2. The 2014 OpenFlights extract (`data/raw/lookups/airlines.csv`) is
   demoted to a cross-check column. It is never again a nation source.
3. The `icao` match tier in `code/02_build/02_carrier_nation.py` is
   deleted as a bug, not excluded as a stratum: it matched BTS's own
   Q-suffixed placeholder codes against real ICAO designators.
4. Nationality comes from a human-signed, dated, sourced table keyed by
   ICAO three-letter designator (OpenSky callsign prefix) with IATA code
   and BTS `AIRLINE_ID` as secondary keys, built in an attended session
   with web access (Round 2, task ATT-02) and placed by the human under
   `data/raw/lookups/`. Each entity maps to the successor state's code
   for the whole sample (territory changes go in a note column); an
   entity's nation is time-invariant unless a `valid_from`/`valid_to`
   pair says otherwise.
5. FIX-02 stays parked, and the Round-1 nation-level reporting embargo
   stays in force, until that table exists and FIX-02 is re-run once on
   the precision reading (Round 2, task FIX-06).

**Reasoning (human).** T-100 carries no foreign airborne time, so the
mapping matters here only for volume outcomes and, more importantly, for
the OpenSky phase, which needs the same table under a different key.
Build it once, later, scoped to the corridors actually analysed. Evidence
packet: `carrier_nation_*.csv` in `rounds/round-1-t100-panel/` and the
seed worksheet regenerated by `code/02_build/02a_curation_worksheet.py`.

**State.** DECIDED (human).

---

## D-02 — RULING (human, 2026-09-04) — DECIDED: (C)

**Ruling.** Accelerate Phase 2: OpenSky ADS-B becomes the primary source
for the carrier-nation wedge. Round 2 (`rounds/round-02-opensky-extract/`)
is the commissioned vehicle, including the attended extract sessions
under T1–T9 and the attended web session for the carrier table. Options
(A), (B) and (D) remain parked, not rejected; T-100 volume fields are
reused in Round 2 only as a capture-rate benchmark for the ADS-B extract.

**State.** DECIDED (human). D-04, D-05 and D-06 remain DECISION-PENDING.
